...
public class LogisticaService {
    ...
    /**
     * Crea una Ruta + Tramos origen-destino para la Solicitud indicada.
     */
public Ruta crearRutaParaSolicitud(CrearRutaRequestDTO request) {
    ...
    validarExistenciaSolicitud(idSolicitud);
    ...
    // 3) Armar la lista de depósitos en orden y validar su existencia
    List<Deposito> depositosEnRuta = construirListaDepositos(request.getPuntosIntermedios());
    int cantidadDepositos = depositosEnRuta.size();
    int cantidadTramos = (cantidadDepositos == 0) ? 1 : (cantidadDepositos + 1);


    // 4) Crear o actualizar Ruta
    ...
    if (depositosEnRuta.isEmpty()) {
        // CASO A: RUTA DIRECTA (ORIGEN -> DESTINO)
        TramoCalculoInfo info = calcularInfoTramo(
        ...
        costoTotalEstimado = costoTotalEstimado.add(info.costoTramo);
        tiempoTotalMin = tiempoTotalMin.add(info.duracionMin);

        // ✅ Llama al método sin asignar el resultado
        crearYGuardarTramo(
                ruta,
                null,
                null,
        ...
        TramoCalculoInfo info1 = calcularInfoTramo(
                origenDTO.getLatitud(),
                origenDTO.getLongitud(),
        ...
        costoTotalEstimado = costoTotalEstimado.add(info1.costoTramo);
        tiempoTotalMin = tiempoTotalMin.add(info1.duracionMin);
        ...
        TramoCalculoInfo infoLast = calcularInfoTramo(
        ...
        costoTotalEstimado = costoTotalEstimado.add(infoLast.costoTramo);
        tiempoTotalMin = tiempoTotalMin.add(infoLast.duracionMin);
    ...
    /**
     * Genera una lista de rutas alternativas (incluyendo la directa) con estimaciones de costo y tiempo.
     */
    ...
    /**
     * Asigna un camión a un tramo, realizando validaciones de capacidad.
     */
    public Tramo asignarCamionATramo(Long idTramo, String patenteCamion) {
        ...
        // 2) Traer SOLICITUD y CONTENEDOR asociados
        ...
        ContenedorDTO contenedorDTO = obtenerContenedorDeSolicitudValidado(idSolicitud);

        // ======================
        // 3) Validar peso y volumen
        // ======================
        ...
        // ======================
        // 4) Si todo OK → asignar camión al tramo
        // ======================
    ...
    /**
     * Estructura de datos temporal para encapsular los resultados de un cálculo de tramo.
     */
    private static class TramoCalculoInfo {
        BigDecimal distanciaKm;
        BigDecimal duracionMin;
        BigDecimal costoTramo;

        public TramoCalculoInfo(BigDecimal distanciaKm, BigDecimal duracionMin, BigDecimal costoTramo) {
    ...
    /**
     * Combina la llamada a OSRM y el cálculo de la estimación de costos/tiempo.
     * Repetitivo en 'crearRutaParaSolicitud' y 'calcularRutaAlternativa'.
     */
    private TramoCalculoInfo calcularInfoTramo(
        ...
        return new TramoCalculoInfo(distanciaKm, duracionMin, costoTramo);
    }
    
    /**
     * Arma y guarda un Tramo con la información calculada.
     * Repetitivo en 'crearRutaParaSolicitud'.
     */
    private Tramo crearYGuardarTramo(
            Ruta ruta,
        ...
        tramo.setTiempoEstimado(tiempoEstimado);

        return tramoRepository.save(tramo);
    }
    
    /**
     * Valida la existencia de la Solicitud y maneja errores de conectividad/respuesta.
     * Repetitivo en 'crearRutaParaSolicitud' y 'generarRutasAlternativas'.
     */
    private void validarExistenciaSolicitud(Long idSolicitud) {
    ...
    /**
     * Arma la lista de Depósitos y valida su existencia.
     * Repetitivo en 'crearRutaParaSolicitud'.
     */
    private List<Deposito> construirListaDepositos(List<PuntoRutaDTO> puntosIntermedios) {
        List<Deposito> depositosEnRuta = new ArrayList<>();

        if (puntosIntermedios != null) {
            for (PuntoRutaDTO p : puntosIntermedios) {
     ...
     * Único punto de contacto con OSRM.
     */
    ...
    /**
     * Calcula una ruta alternativa (incluyendo sus tramos) y arma el DTO de respuesta.
     * Se usa en 'generarRutasAlternativas'.
     */
    ...
    /**
     * Intenta obtener el Contenedor de una Solicitud, manejando errores de Solicitud/Contenedor no encontrados.
     * Repetitivo en 'asignarCamionATramo' y 'cerrarSolicitudConCostosYTiempo'.
     */
    private ContenedorDTO obtenerContenedorDeSolicitudValidado(Long idSolicitud) {
        SolicitudDTO solicitudDTO = obtenerSolicitud(idSolicitud);
     ...
     */
    private SolicitudDTO obtenerSolicitud(Long idSolicitud) {
        String urlSolicitud = solicitudBaseUrl + "/" + idSolicitud;
        ...
        return respSolicitud.getBody();
    }

    /**
     * Llama a la API de Solicitud para actualizar la estimación de costo y tiempo.
     */
    ...
    private void cerrarSolicitudConCostosYTiempo(Ruta ruta, List<Tramo> tramos) {
        ...
        // 3) Traer contenedor para sumar tarifa base
        ContenedorDTO contenedorDTO = obtenerContenedorDeSolicitud(idSolicitud);
    ...
    /**
     * Intenta obtener el Contenedor de una Solicitud sin lanzar excepciones en caso de error de conexión/API.
     * Se usa en 'crearRutaParaSolicitud' y 'cerrarSolicitudConCostosYTiempo'.
     */
